...
a diverse, coherent set of PII into the anonymized patient messages while keeping their original
wording and medical content intact, and a gold-standard target response is then generated for each
query.

> All PII in Med-PCD is synthetic and does not correspond to any real individual.
...
A single instance (`med-pcd-00001`, abbreviated). The queries keep the informal phrasing of the
original MedDialog messages.
...
2. **PII injection.** Each anonymized patient message is passed to an LLM with a few-shot prompt
   (from the paper's Appendix A) that weaves in a diverse set of PII — names, DOBs, MRNs, phones,
   emails, doctors, clinics, departments, insurance, pharmacies, dates, addresses, cities/states,
   travel and workplace details. The injection keeps the original phrasing, leaves every medical
   fact and timeline unchanged, and stays internally consistent (for example, an injected DOB
   agrees with any age stated in the text).
3. **Gold responses.** Each PII-injected query is sent to a powerful model to produce the target
   response `r*`, representing the quality a privacy-preserving system aims to match.

In the paper, a human validation study over 240 instances found 98.8% judged as a "pass" for
contextual coherence and logical consistency (Fleiss' κ = 0.89).

---

## Ethical Use

- Med-PCD builds on MedDialog, a publicly available and already-anonymized resource, so no real
  patient information is present.
- All injected PII is synthetic and generated by an LLM; it does not refer to real people.
- The dataset is intended for research on privacy-preserving NLP systems. Please use it responsibly
  and in line with the source data's terms.
